...
__all__ = ['_minorb_bin', '_minorb_bin_withcoll', '_strip_comments',
           'clean_remote_processes', 'start_remote_server', 'token_connect', 'remote_home']
...
def connector_to_rpyc(data, THIS_PATH):
    ...
    c.modules.sys.path.append(
        c.modules.os.path.expanduser('~/local/symorb/py'))
    ...
    # sys.stderr.write("to connect to %s\n" % TODO_PATH)
    PP = c.modules.subprocess.Popen(LD_PRELOAD + " " + TODO_PATH, shell=True,
                                    bufsize=8192, stdin=PIPE, stdout=PIPE, stderr=PIPE, close_fds=True)
...
def get_remote_processes(remote_host):
    QQ = subprocess.Popen("ssh  %s ls %s" % (
        remote_host, ".symorb/rpyc_server*"), shell=True, bufsize=8192, stdout=PIPE, stderr=PIPE)
...
def clean_remote_processes(remote_host):
    for x in get_remote_processes(remote_host):
        QQ = subprocess.Popen("ssh  %s rm -f .symorb/rpyc_server-%s" %
                              (remote_host, x), shell=True, bufsize=8192, stdout=PIPE, stderr=PIPE)
        res, err = QQ.communicate()
        QQ = subprocess.Popen("ssh  %s kill %s" % (
            remote_host, x), shell=True, bufsize=8192, stdout=PIPE, stderr=PIPE)
        res, err = QQ.communicate()


def start_remote_server(remote_host):
    START_COMMAND = ["ssh", remote_host +
                     " /home/ferrario/local/symorb/bin/rpyc_server.sh "]
    ...
    PP = subprocess.Popen(" ".join(START_COMMAND), shell=True,
                          bufsize=8192, stdout=PIPE, stderr=PIPE)
    res, err = PP.communicate()
    res = res.strip()
    port_number, shared_token = res.split(":")
    print(("(=> running remote server on %s:%s ...)" %
          (remote_host, port_number)))
