...
_TMPFILE_NAME = "/tmp/minpath_%i%i%i" % (os.geteuid(),
...
"""
    ...
    def __init__(self, *args):
        ...
        for i in range(self.NOB):
            for j in range(0, self.steps+2):
                self.gamma.append([0.0 for x in range(self.dim)])
        self.setgammadata()
        self.a = 0
        self.h = 0
        self.log = ''
    ...
    def view(self, opts=''):
        ...
        os.system(_ORBVIEW_PATH + (' %s ' % opts) +
                  _TMPFILE_NAME)  # TODO: change!
    ...
    def reshape(self, steps):
        ...
        for i in range(self.dim):
            zeros.append(0.0)
        index = 0
        array = list(range(self.NOB))
        for i in range(self.NOB):
            array[i] = list(range(self.steps+2))
    ...
    def parsegammadata(self):
        ...
        if len(alllines) < 3:
            ...
            alllines = alllines[:-1]
        (self.NOB, self.dim, self.steps) = list(map(int, re.match(
            ' *(\d+) +(\d+) +(\d+)', alllines[0]).groups()))
        ...
        for line in alllines[3:3+self.NOB]:
            self.masses.append(
                float(re.match(" *(%s)" % float_regex, line).groups()[0]))
        self.writemasses()
        for line in alllines[self.NOB+3:]:
            numline = list(map(float, re.match(numline_regex, line).groups()))
...
""" % (self.NOB, self.dim, self.steps, self.omega[0], self.omega[1], self.omega[2], self.ALPHA)
        ...
        for vect in self.gamma:
            self.gammadata = self.gammadata + \
                (" %.24e" * self.dim) % tuple(vect) + "\n"
        return
    ...
    def __add__(self, other):
        if type(other) == type(self) and self.symfilename == other.symfilename and self.steps == other.steps and self.masses == other.masses and self.omega == other.omega:
            ...
            for line in range(len(result.gamma)):
                for i in range(self.dim):
                    result.gamma[line][i] = self.gamma[line][i] + \
                        other.gamma[line][i]
            result.setgammadata()
        else:
            result = None
